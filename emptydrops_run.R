...
parser$add_argument("counts", type="character", help="Counts file as .csv with no headers")
parser$add_argument("lower", type="integer", help="Lower limit of counts to consider initial empty droplets")
...
fname <- tail(strsplit(as.character(args$counts), "/")[[1]], n=1)
...
if (tail(strsplit(as.character(fname), "\\.")[[1]], 1) == "csv"){
    ...
    x <- read.csv(args$counts, header=F)
...
# read in 10X .mtx files
    ...
    message(paste0("Reading in data for '",fname,"'"))
    sce <- read10xCounts(args$counts)
    outname <- strsplit(fname, "/")[[1]][1]
}

message(paste0("Calculating global counts metrics for '",outname,"'"))
final <- counts(sce)
stats <- barcodeRanks(final, lower=args$lower)
...
# plotting deviances and cell labels
